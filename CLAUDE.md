...
webview-ui/src/               — React + TypeScript (Vite)
  ...
  components/
    ...
    CostumePanel.tsx           — Modal: pick palette (0-5) + hue shift for an agent; previews idle[0] sprite
    AgentLabels.tsx            — Name/status labels rendered above characters
  office/
    ...
    sprites/
      spriteData.ts           — Pixel data: characters (CharacterSprites interface, buildCircusSprites, named + stream avatar systems), furniture, tiles, bubbles
      spriteCache.ts          — SpriteData → offscreen canvas, per-zoom WeakMap cache, outline sprites, hit flash
      streamAvatarController.ts — Minion/enemy sprite sheets: load, getAvatarKeys(), pickRandom(), getFrame(key, animType, frameIdx)
    editor/
...
- Match the established palette before generating new tiles (avoid overly dark or off-theme colors)
- Confirm style direction (e.g., dungeon vs circus vs arena) BEFORE generating full tilesets
- Show one small sample first for stylistic approval, then iterate to full set
...
**Character sprites**: One main PNG `assets/characters/char_cc.png` (336×144: 7 frames × 48px wide, 3 direction rows × 48px tall). The same data is broadcast to all 6 palette slots. Row 0 = down, Row 1 = up, Row 2 = right. Frame order: walk1, walk2, walk3, type1, type2, read1, read2. `buildStandardSprites()` maps these to the `CharacterSprites` interface (idle/walk/sit/attack/celebrate tuples). Named characters (`char_<name>.png`) are loaded separately and resolved by character `.name` field in the renderer. Circus-format sheets (192×240, 4 cols × 5 rows of 48×48) are auto-detected and parsed by `buildCircusSprites()` into the same interface. Circus frame layout: Row 0 = idle (2 frames), Row 1 = walk (4), Row 2 = sit (2), Row 3 = attack (3), Row 4 = celebrate (3). Hardcoded template fallback when PNG not loaded. When `hueShift !== 0`, `hueShiftSprites()` applies `adjustSprite()` to all frames before caching.

**Stream avatars (minions/enemies)**: Non-`char_*` PNGs in `assets/characters/` are loaded as stream avatars. The loader auto-detects a 4-col × 5-row grid (preferred), else falls back to GCD-based square frames. Each avatar stores `rows[rowIdx][frameIdx]` — Row 0 = idle, Row 1 = walk, Row 2 = fight, Row 3 = sleep, Row 4 = sit. `streamAvatarController.ts`: `loadStreamAvatars()`, `getAvatarKeys()`, `pickRandom()`, `getFrame(key, animType, frameIdx)`. Used by renderer for minions via `getAvatarFrame()`. Key naming: filename lowercased, spaces → `_`, `.png` stripped.

**Arena / Booth system**: Two `FightBooth` entries (`red`/`blue`) are initialized via `officeState.initBooths(avatarKey1, avatarKey2)`. Each booth links a special chair seat uid (`booth-red-chair`/`booth-blue-chair`) to a `Minion` at a fixed tile. When an agent assigned to a booth seat becomes active, its minion switches to `'fight'` state; inactive → `'idle'`. Minion frame is cycled by `updateMinions(dt)`. Renderer draws minions via `getAvatarFrame(minion.avatarKey, animType, minion.frame)`. `pickRandom()` in `streamAvatarController.ts` exists for random avatar selection but no UI shuffle button is wired yet. `officeState.updateMinions()` is called each game-loop tick. Minions are not persisted — re-initialized after layout loads.

**Load order**: `characterSpritesLoaded` (includes `namedCharacters` + `streamAvatars`) → `floorTilesLoaded` → `wallTilesLoaded` → `furnitureAssetsLoaded` (catalog built synchronously) → `layoutLoaded`.
...
- After modifying tile/sprite assets, ALWAYS rebuild AND copy assets to `dist/` before reporting completion
- Verify the asset actually loads at runtime, don't just confirm the code change compiles
- For sprite extraction, never assume uniform grid sizing — use bounding-box/gap-scan detection

## Node Environment

- This project requires Node 24+ (Vite 8). If you encounter rolldown native binding errors, do a clean `rm -rf node_modules && npm install`

## Blender MCP Workflow

- Always use Blender MCP tools directly — never fall back to writing standalone Python scripts
- If MCP tools aren't available, STOP and tell the user rather than substituting scripts
- For character models, request a reference image upfront before generating
...
- **Extension backend**: `src/constants.ts` — timing intervals, display truncation limits, PNG/asset parsing values (`CHAR_FRAME_W/H = 48`, `CHAR_FRAMES_PER_ROW = 7`, `CHAR_COUNT = 6`), VS Code command/key identifiers
